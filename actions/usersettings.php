...
else if ($user = $this->GetUser())
{
	...
	if (isset($_REQUEST["action"]) && ($_REQUEST["action"] == "changepass"))
	{
			...
			$password = $_POST["password"];			
                        if (preg_match("/ /", $password)) $passerror = ERROR_NO_BLANK;
			else if (strlen($password) < PASSWORD_MIN_LENGTH) $passerror = sprintf(ERROR_PASSWORD_TOO_SHORT, PASSWORD_MIN_LENGTH);
	...
	<br />
	<?php echo $this->Format(QUICK_LINKS); ?>
	<?php
	print($this->FormClose());
}
else
{
	...
	if (isset($_REQUEST["action"]) && ($_REQUEST["action"] == "login"))
	{
		...
		else
		{
			...
			if (strlen($name)==0) $error = ERROR_EMPTY_USERNAME;
			elseif (!$this->IsWikiName($name)) $error = ERROR_WIKINAME;
			elseif ($this->ExistsPage($name)) $error = ERROR_RESERVED_PAGENAME;
			elseif (strlen($password)==0) $error = ERROR_EMPTY_PASSWORD;
			elseif (preg_match("/ /", $password)) $error = ERROR_NO_BLANK;
			elseif (strlen($password) < PASSWORD_MIN_LENGTH) $error = sprintf(ERROR_PASSWORD_TOO_SHORT, PASSWORD_MIN_LENGTH);
			elseif (strlen($confpassword)==0) $error = ERROR_EMPTY_CONFIRMATION_PASSWORD;
			elseif ($confpassword != $password) $error = ERROR_PASSWORD_MATCH;
			elseif (!$email) $error = ERROR_EMAIL_ADDRESS_REQUIRED;
			elseif (!preg_match(VALID_EMAIL_PATTERN, $email)) $error = ERROR_INVALID_EMAIL_ADDRESS;
			else
			{
	...
	elseif  (isset($_REQUEST["action"]) && ($_REQUEST["action"] == "updatepass"))
	{
        	...
        	if (!$this->IsWikiName($name)) $newerror = ERROR_WIKINAME;
	...
	<table>
   	<tr> 
   		<td colspan="2"><?php echo $this->Format(REGISTER_HEADING) ?></td><td></td>
   	</tr>
		<tr>
			<td align="right"></td>
