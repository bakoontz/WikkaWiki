...
$rss_path = $vars['url'];
if ((!$rss_path) && $wakka_vars) $rss_path = $wakka_vars;
...
if (preg_match("/^(http|https):\/\/([^\\s\"<>]+)$/i", $rss_path)) 
{
	...
	echo "<span class='error'><i>Error: Invalid RSS syntax. <br /> Proper usage: {{rss http://domain.com/feed.xml}} or {{rss url=\"http://domain.com/feed.xml\"}}</i></span>";
}
