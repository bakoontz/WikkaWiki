...
if ($user = $this->GetUser())
{
	...
	if ($_REQUEST["alphabetically"] == 1)
	{
		...
		//#dotmg [1 line modified]: changed [tag NOT LIKE 'Comment%'] by [comment_on = '']
		if ($pages = $this->LoadAll("SELECT tag, time FROM ".$this->config["table_prefix"]."pages WHERE user = '".mysql_escape_string($this->UserName())."' AND comment_on = '' ORDER BY tag ASC, time DESC"))
		{
