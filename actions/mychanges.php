...
if ($user = $this->GetUser())
{
	...
	if ($_REQUEST["alphabetically"] == 1)
	{
		...
		if ($pages = $this->LoadAll("SELECT tag, time FROM ".$this->config["table_prefix"]."pages WHERE user = '".mysql_escape_string($this->GetUserName())."' ORDER BY tag ASC, time DESC"))
		{
	...
	else
	{
		...
		if ($pages = $this->LoadAll("SELECT tag, time FROM ".$this->config["table_prefix"]."pages WHERE user = '".mysql_escape_string($this->GetUserName())."' ORDER BY time ASC, tag ASC"))
		{
			foreach ($pages as $page)
			{
