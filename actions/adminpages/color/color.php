...
class WikkaAction_color extends WikkaAction
{
	...
	function process($vars = null)
	{
		...
		if (is_array($vars))
		{
			...
			{
				$value = $this->wakka->htmlspecialchars_ent($value);
				if ($param == 'text')
				{
					$mytext = $this->wakka->htmlspecialchars_ent($value);
				}
				elseif (($param == 'c') || ($param == 'hex') || ($param == 'fg'))
				{
					$fgcolor = $this->color_syntax_is_valid($value);
					if ($fgcolor)
					{
						$style .= "color: $fgcolor; ";
					}
				...
				elseif ($param == 'bg')
				{
					$bgcolor = $this->color_syntax_is_valid($value);
					if ($bgcolor)
					{
