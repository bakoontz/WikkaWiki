...
class Wakka
{
	...
	function SavePage($tag, $body, $note)
	{
		...
		if ($this->HasAccess("write", $tag))
		{
			...
			{
				// create default write acl. 
				$this->SaveACL($tag, "write", $this->GetConfigValue("default_write_acl"));
	...
	function SaveComment($page_tag, $comment)
	{
		...
		$this->Query("insert into ".$this->config["table_prefix"]."comments set ".
			...
			"time = now(), ".
			"comment = '".mysql_escape_string(trim($comment))."', ".
			"user = '".mysql_escape_string($user)."'");
	...
	function LoadACLs($tag, $useDefaults = 1)
	{
	...
	function Run($tag, $method = "")
	{
		...
		$this->LogReferrer();
		$this->ACLs = $this->LoadACLs($this->tag);
