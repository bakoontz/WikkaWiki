...
class Wakka
{
	...
	function LoadRecentlyCommented($limit = 50)
	{
		...
		return $this->LoadAll("select comment_on as tag, max(time) as comment_time, tag as comment_tag, max(concat(time, ']', user)) as comment_user from ".$this->config["table_prefix"]."pages where comment_on != '' group by comment_on order by comment_time desc");
	}
