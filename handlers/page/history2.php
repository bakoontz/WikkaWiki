...
if ($this->HasAccess("read")) {
    ...
    if ($pages = $this->LoadRevisions($this->tag))
    {
		...
		$output='';
                      ...
                      if ($c == 2) {
                            echo "<strong>Most recent edit on <a href=\"".$this->href("", "", "time=".urlencode($pageA["time"]))."\">".$pageA["time"]."</a> by ".$EditedByUser."</strong> <span style=\"color:#888;font-size:smaller;\">[".$page["note"]."]</span></strong><br /><br />\n";
                        }
                        else {
                            // $output .= "<strong>Comparing <a href=\"".$this->href("", "", "time=".urlencode($pageA["time"]))."\">".$pageA["time"]."</a> to <a href=\"".$this->href("", "", "time=".urlencode($pageB["time"]))."\">".$pageB["time"]."</a></strong><br />\n";
                          echo "<strong>Edited on <a href=\"".$this->href("", "", "time=".urlencode($pageA["time"]))."\">".$pageA["time"]."</a> by ".$EditedByUser."</strong> <span style=\"color:#888;font-size:smaller;\">[".$page["note"]."]</span><br /><br />\n";
  	                }                    
   ...
   $bodyE = explode("\n", $pageB["body"]);
   $end_output .= "<strong>Oldest known version of this page was edited on <a href=\"".$this->href("", "", "time=".urlencode($pageB["time"]))."\">".$pageB["time"]."</a> by ".$EditedByUser."</strong> <span style=\"color:#888;font-size:smaller;\">[".$page["note"]."]</span></strong><br /><br />\n";
    $end_output .= $this->Format(implode("\n", $bodyE));
    }
    print($end_output);
