			...
			if (($c <= $max) || !$max)
			{
				...
				if (isset($pageA))
				{
					...
					if ($allowed) {
						...
						if ($c == 2) {
							$output .= "<strong>Most recent edit on <a href=\"".$this->href("", "", "time=".urlencode($pageA["time"]))."\">".$pageA["time"]."</a> by ".$EditedByUser."</strong> <span style=\"color:#888;font-size:smaller;\">[".$page["note"]."]</span></strong><br />\n";
						}
						else {
							$output .= "<strong>Edited on <a href=\"".$this->href("", "", "time=".urlencode($pageA["time"]))."\">".$pageA["time"]."</a> by ".$EditedByUser."</strong> <span style=\"color:#888;font-size:smaller;\">[".$page["note"]."]</span><br />\n";
						}                    
