...
if ($this->HasAccess("read")) {
	...
	if ($pages = $this->LoadRevisions($this->tag))
	{
		...
		$output .= "<td><input value=\"1\" type=\"checkbox\" checked name=\"fastdiff\"/>\n".$this->Format("Simple Diff")."</td>";
		$output .= "</tr>\n";
		$output .= "</table>\n";
		$output .= "<table border=\"0\" cellspacing=\"0\" cellpadding=\"1\">\n";
		...
		foreach ($pages as $page)
		{
			...
			if (($c <= $max) || !$max)
			{
				...
				$output .= "<td>&nbsp;by ".$this->Format($page["user"])." <span style=\"color:#888;font-size:smaller;\">[".$page["note"]."]</span></td>";
				$output .= "</tr>\n";
			}
		}
		$output .= "</table><br />\n";
