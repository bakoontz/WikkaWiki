...
if ($this->IsAdmin())
{
    if ($_POST)
    {
        ...
        // delete from wikka_pages where tag=
        // delete from wikka_comments where tag=
        // delete from wikka_links where from_tag =
        // delete from wikka_acls where page_tag =
        // delete from wikka_referrers where page_tag =

        $delResult = $this->LoadSingle("delete from ".$this->config["table_prefix"]."pages where tag = '".mysql_escape_string($tag)."'");
        $delResult = $this->LoadSingle("delete from ".$this->config["table_prefix"]."comments where page_tag = '".mysql_escape_string($tag)."'");
