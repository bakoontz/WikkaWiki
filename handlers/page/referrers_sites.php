...
if ($this->GetUser()) {
	if ($referrers)
	{
		...
		foreach ($referrer_sites as $site => $site_count)
		{
			...
			print("<td valign=\"top\">" . (($site != "unknown") ? "<a href=\"http://".htmlspecialchars($site)."\">".htmlspecialchars($site)."</a>" : $site) . "</a> ".($IsAdmin ? "[<a href=\"".$this->href("delete_referrer", "", "spam_site=").htmlspecialchars($site)."&redirect=".$this->GetMethod()."\">Blacklist</a>]" : "")."</td>");
			print("</tr>\n");
		}
		print("</table>\n");
	}
