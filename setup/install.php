...
case "0":
  ...
  test("Adding admin user...", 
    @mysql_query("insert into ".$config["table_prefix"]."users set name = 'WikkaInstaller', password = md5('".mysql_escape_string($_POST["password"])."'), email = '".$config["admin_email"]."', signuptime = now()", $dblink), "Hmm!", 0);

	test("Adding some pages...", 1);
	mysql_query("insert into ".$config["table_prefix"]."pages set tag = '".$config["root_page"]."', body = '".mysql_escape_string("Welcome to your Wikka site! Click on the \"Edit page\" link at the bottom to get started.\n\nAlso don't forget to visit [[Wikka:HomePage WikkaWiki]]!\n\nUseful pages: FormattingRules, OrphanedPages, WantedPages, TextSearch.")."', user = '".$config["admin_users"]."', owner = '".$config["admin_users"]."', time = now(), latest = 'Y'", $dblink);
